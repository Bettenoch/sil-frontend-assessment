	...
	return {
	  queryFn: async () => {
		const promises = userIds.map((userId) =>
		  UserAlbumsService.getAllUserAlbums({ userId }).then(
			(albums) => ({ userId, count: albums.data.length })
		  )
		);
					...
					>
					  ...
					  >
						...
						<Box
						  ...
						  justifyContent={"center"}
						>
						  <Flex gap={"2"} position={"relative"} direction={"column"}>
							...
							<Text fontSize={"md"} fontWeight={"bold"} textDecor={"underline"} color={useColorModeValue("#219EBC","#0855B1")}>{user.username}</Text>
							<Text>Albums: <Box as="span" fontSize={"xl"} fontWeight={"semibold"}> {albumCount || 0}</Box></Text>
						  </Flex>
						  <Box w="full" alignItems={"center"} justifyContent={"center"} gap={1} display={"flex"} flexDirection={{base:"column", md:"row"}}>
							<Text color={useColorModeValue("#219EBC","#010E54")}>
								Joined
							</Text>
						  	<Text fontSize={"sm"}> ({user.created_at && formatDate(user.created_at)})</Text>
						  </Box>
						</Box>
					  </Box>
					</Link>
